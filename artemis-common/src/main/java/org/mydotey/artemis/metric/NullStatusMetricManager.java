...
public class NullStatusMetricManager<T> implements StatusMetricManager<T> {
    ...
    public static <T> NullStatusMetricManager<T> getInstance(Class<T> statusClass, final T status) {
        return (NullStatusMetricManager<T>) _managerCache.computeIfAbsent(statusClass,
            k -> new NullStatusMetricManager<T>(status));
    }

    public static final NullStatusMetricManager<Double> DOUBLE_NULL_STATUS_METRIC_MANAGER = NullStatusMetricManager
        .getInstance(Double.class, 0.0);
    ...
    private MetricManagerConfig<StatusMetric<T>> _managerConfig = new MetricManagerConfig<>(
        NullStatusMetricReporter.<T>getInstance());
