 ...
 */
public class BusinessDaoTest extends ArtemisTest {
    ...
    public void testCreateServiceRouteRules() {
        ...
        assertServiceRouteRule(serviceId, activeRouteRuleInfos, GroupRepository.RouteRuleStatus.ACTIVE,
            GroupRepository.GroupStatus.ACTIVE);
        assertServiceRouteRule(serviceId, routeRuleInfos, GroupRepository.RouteRuleStatus.INACTIVE,
            GroupRepository.GroupStatus.INACTIVE);
    ...
    public void testOperateGroupOperation() {
        ...
        List<GroupOperationModel> operationModels = groupOperationDao.query("group_id=?",
            Lists.newArrayList(Long.toString(newGroup.getId())));
        ...
        List<GroupOperationLog> logs = groupOperationLogDao
            .select(new GroupOperationLogModel(newGroup.getId(), operationContext.getOperation(),
                operationContext.getOperatorId()), false);
        ...
        logs = groupOperationLogDao.select(
            new GroupOperationLogModel(newGroup.getId(), operationContext.getOperation(),
                operationContext.getOperatorId()),
            true);
        Assert.assertEquals(1, logs.size());
        Assert.assertEquals(operationContext.getOperation(), logs.get(0).getOperation());
        Assert.assertEquals(operationContext.getToken(), logs.get(0).getToken());
    ...
    private void assertServiceRouteRule(String serviceId, List<RouteRuleInfo> routeRuleInfos, String routeRuleStatus,
        String groupStatus) {
        ...
        for (RouteRuleModel routeRuleModel : routeRuleDao.select(filter)) {
            ...
            for (String groupKey : routeRuleGroups.get(key)) {
                RouteRuleGroupModel routeRuleGroupModel = newRouteRuleGroups.get(routeRuleModel.getId())
                    .get(newGroups.get(groupKey).getGroupId());
                Assert.assertEquals(groupWeights.get(groupKey).getWeight(), routeRuleGroupModel.getUnreleasedWeight());
                if (GroupRepository.GroupStatus.ACTIVE.equals(groupStatus)
                    && GroupRepository.RouteRuleStatus.ACTIVE.equals(routeRuleStatus)) {
