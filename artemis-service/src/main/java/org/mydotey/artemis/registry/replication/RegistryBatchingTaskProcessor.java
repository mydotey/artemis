 ...
 */
public class RegistryBatchingTaskProcessor
    implements TaskProcessor<RegistryReplicationTask, List<RegistryReplicationTask>> {
        ...
        try {
            ...
            for (RegistryReplicationTask task : work) {
                for (Class<?> clazz : RegistryReplicationTool.supportedSubTaskTypes()) {
                    ...
                    if (task.getClass() == clazz) {
                        String serviceUrl = task.serviceUrl() == null ? StringExtension.EMPTY : task.serviceUrl();
            ...
            for (Class<?> clazz : RegistryReplicationTool.supportedSubTaskTypes()) {
                ...
                for (String serviceUrl : serviceUrlInstancesMap.keySet()) {
                    ...
                    Map<Instance, Long> instanceExpiryTimeMap = subTaskTypeInstanceExpiryTimeMap.get(clazz);
                    List<RegistryReplicationTask> failedTasks = RegistryReplicationTool.replicate(clazz, serviceUrl,
                        instances, instanceExpiryTimeMap);
